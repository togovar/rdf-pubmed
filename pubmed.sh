...
# 削除されたPMIDのリスト作成
ls -d $WORK/updatefiles/pubmed*.xml.gz | xargs -P10 -n1 -Ifile ruby pubmed_delete_citation.rb file delete_pmids.txt


# 変換の実行
ls -d $WORK/**/pubmed*.xml.gz | xargs -P10 -n1 -Ifile ruby /pubmed_converter.rb file delete_pmids.txt 
